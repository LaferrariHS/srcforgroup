...
void GraphPlanner::initialize(std::string name)
{
  if (!initialized_)
  {
    ...
    // planner name
    private_nh.param("planner_name", planner_name_, (std::string) "lazy_theta_star");
    if (planner_name_ == "lazy_theta_star")
    ...
    plan_pub_ = private_nh.advertise<nav_msgs::Path>(planner_name_ + "_plan", 1);
